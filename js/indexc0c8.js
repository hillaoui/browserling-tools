...
Site.pricing.lang = {
    ...
    START_USING_BROWSERLING: "Start using Browserling",
...
Site.pricing.showLoginError = function(plan, message) {
    var element = plan.querySelector(".scroll-auth .auth-login .plan-generic-error");
    return Site.pricing.showGenericError(plan, element, message);
}
Site.pricing.showRegisterError = function(plan, message) {
    var element = plan.querySelector(".scroll-auth .auth-register .plan-generic-error");
    return Site.pricing.showGenericError(plan, element, message);
}
...
Site.pricing.showGenericError = function(plan, element, message) {
    ...
    if (!document.querySelector("section.plans").classList.contains("has-selected")) {
...
Site.pricing.getPlanPrice = function(plan) {
    ...
    var billing  = document.querySelector("section.plans").getAttribute("data-billing");
    ...
    if (plan.classList.contains("plan-free")) {
        ...
        if (!planSettings) {
            ...
            var teamUsers    = parseInt(planSettings.querySelector(".plan-team-size-selector input:checked").value);
            var deltaPerUser = parseInt(plan.getAttribute("data-price-delta"));

            var total = (basePrice + ((teamUsers - 2) * deltaPerUser)) * multiplier * discountMultiplier;
            if (discountMultiplier != 1) {
                total = total.toFixed(2);
            }
...
Site.pricing.updateFrontSide = function(plan) {
    ...
    if (priceOnFront) {
        if (plan.classList.contains("plan-team")) {
            priceOnFront.querySelector(".amount").textContent = price.textFront.replace("$", "");
        }
        else {
...
Site.pricing.updatePlan = function(plan) {
    var backSide = plan.querySelector(".plan-side-back");
    var actionButton = backSide ? backSide.querySelector(".plan-button button span") : null;
    ...
    if (state == "closed") {
        ...
        var input = backSide.querySelector(".scroll-auth .auth-login form input[type=text]");
        setTimeout(function(){
            input.focus();
        }, 501);
    }
        ...
        var input = backSide.querySelector(".scroll-auth .auth-register form input[type=text]");
        setTimeout(function(){
            input.focus();
        }, 501);
    }
    else if (state == "complete") {
        actionButton.textContent = Site.pricing.lang.START_USING_BROWSERLING;
    }
    else if (state == "payment") {
        ...
        if (!planSettings) {
            ...
            //
            statusText.textContent = Site.pricing.lang.ORDER_REGULAR_SUBTITLE.format(price.text, price.timespan);

            // Update PayPal subscription link to monthly/yearly
            //
            var container = document.querySelector("section.plans");
            var devPlanPayPalLink = 'https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=MJNKSB2GRDBQC';
            if (container.getAttribute("data-billing") == "yearly") {
                devPlanPayPalLink = 'https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=ZY2UUNQAL8RV4';
            }
            ...
            //
            var users = parseInt(planSettings.querySelector(".plan-team-size-selector input:checked").value);
            var teamSize = planSettings.querySelector(".team-size-number");

            teamSize.textContent = Site.pricing.lang.ORDER_MULTIUSER_USERS.format(users);
            statusText.textContent = Site.pricing.lang.ORDER_MULTIUSER_SUBTITLE.format(users);
...
Site.pricing.initializePlan = function(which) {
    ...
    var couponButton = backSide.querySelector(".plan-side-scroll.scroll-pay .plan-payment-coupon .icon-check");
    couponButton.addEventListener("click", function(){
        ...
        var coupon = backSide.querySelector(".plan-side-scroll.scroll-pay .plan-payment-coupon .plan-coupon-code").value;
        ...
        Site.pricing.applyCouponCode(plan, coupon, function (isSuccess, data) {
            if (isSuccess) {
                ...
                Site.pricing.showCouponSuccess(plan, Site.pricing.lang.COUPON_APPLIED.format(data.description));
    ...
    actionButton.addEventListener("click", function(){
        ...
        if (backSide.classList.contains("state-payment")) {
            function handlePayment () {
                ...
                function stripeResponseHandler (status, response) {
                    ...
                    if (which == "team") {
                        var planSettings = backSide.querySelector(".plan-title .plan-settings");
                        var teamSize = parseInt(planSettings.querySelector(".plan-team-size-selector input:checked").value);
        ...
        // logging in
            ...
            var loginWrapper = backSide.querySelector(".plan-side-scroll.scroll-auth .auth-login");
            ...
            Site.account.login(email, password, function (status, response) {
                if (status != 200) {
                    Site.pricing.showLoginError(plan, Site.pricing.lang.GENERIC_SERVER_ERROR);
                ...
                try {
                    ...
                    Site.pricing.showLoginError(plan, Site.pricing.lang.JSON_LOGIN_PARSE_ERROR);
                    actionText.textContent = Site.pricing.lang.SIGN_IN_ACTION;
                    return;
                }
            });
        ...
        // register
            ...
            var registerWrapper = backSide.querySelector(".plan-side-scroll.scroll-auth .auth-register");
            var email = registerWrapper.querySelector('.plan-auth-email').value;
            var password1 = registerWrapper.querySelector('.plan-auth-password').value;
            var password2 = registerWrapper.querySelector('.plan-auth-confirm-password').value;
            Site.account.register(email, password1, password2, function (status, response) {
                if (status != 200) {
                    Site.pricing.showRegisterError(plan, Site.pricing.lang.GENERIC_SERVER_ERROR);
    ...
    var teamSelectors = backSide.querySelectorAll(".plan-team-size-selector input[type=radio]");
    var selector = null;
...
Site.pricing.initialize = function() {
    ...
    var tryForFree = container.querySelector(".plan-free .plan-side-front .plan-button");
    tryForFree.addEventListener("click", Site.pricing.scrollToBrowserSelection);
    var startUsingBrowserling = container.querySelector(".plan-developer .scroll-complete-start-using-browserling");
    startUsingBrowserling.addEventListener("click", Site.pricing.scrollToBrowserSelection);
    ...
    var billMonthly = container.querySelector(".pricing-period-selector > .pricing-period-monthly");
    var billYearly  = container.querySelector(".pricing-period-selector > .pricing-period-yearly");
...
$(function () {
    ...
    var browserVersions = {
        ...
        'win7' : {
            ...
            'safari' : [ '4.0', '5.0.5', '5.1' ],
            'opera' : [ "next", 10, 10.5, 11, 11.5, 11.6, 12 ].concat(range(15, 58)).concat([60, 62]).concat(range(63,71))
        },
        'win8' : {
            'ie' : [ 10 ],
    ...
    $(".testimonial-pic").click(function (ev) {
        ...
        $("#testimonial-" + idx).css('display', 'inline-block')
